...
class mjpgServer(BaseHTTPRequestHandler):
	...
	def do_GET(self):
		if self.path == '/mjpg':
			...
			self.end_headers()
			while True:
				global broadcastImage
				if (broadcastImage is None):
					time.sleep(1)
					continue
				jpg = cv2.imencode('.jpg', broadcastImage)[1]
			...
			self.wfile.write(bytes('<h1>{0!s}[{1!s}]:{2!s}</h1>'.format(hostname, ip, port), "utf-8"))
			self.wfile.write(bytes('<img src="http://{}:{}/mjpg"/>'.format(ip, port), "utf-8"))
			self.wfile.write(bytes('<p>{0!s}</p>'.format(self.version_string()), "utf-8"))
			self.wfile.write(bytes('<p>MJPG Stream</p>', "utf-8"))
...
def initServer():
	...
	global server
	server = HTTPServer((ip, 1181), mjpgServer)
	server.serve_forever()

# main thread
def main():
	thread = Thread(target = initServer)
	thread.start()
	while True:
		...
		if check:
			...
			global broadcastImage
			broadcastImage = imutils.resize(image, 320, 240)
